...
int PD0Parser::extractPacket(uint8_t const* buffer, size_t size,
  ...
  uint16_t checksum = 0;
  for (int i = 0; i < ensemble_size; ++i) checksum += buffer[i];

  uint16_t msg_checksum =
      le16toh(*reinterpret_cast<uint16_t const*>(buffer + ensemble_size));
  ...
  if (sizeof(raw::Header) + header.msg_count * 2 > ensemble_size) return -2;
  ...
  for (int i = 0; i < header.msg_count; ++i) {
    if (expected_offset != 0 && offsets[i] != expected_offset) return -2;

    uint32_t msg_id =
        le16toh(*reinterpret_cast<uint16_t const*>(buffer + offsets[i]));
    uint32_t msg_size = getSizeOfMessage(msg_id);
    if (msg_size != 0) expected_offset = offsets[i] + msg_size;
  }
  return total_size;
}

int PD0Parser::getSizeOfMessage(uint16_t msg_id) const { return 0; }
...
void PD0Parser::parseEnsemble(uint8_t const* buffer, size_t size) {
  ...
  if (sizeof(raw::Header) + header.msg_count * 2 > size)
    throw std::runtime_error("not enough bytes for " +
                             lexical_cast<string>((int)header.msg_count) +
                             " messages");
...
static Sensors parseSensors(uint8_t bitfield) {
  Sensors result;
  result.calculates_speed_of_sound =
      bitfield & raw::PD0_CALCULATE_SPEED_OF_SOUND;
...
void PD0Parser::parseFixedLeader(uint8_t const* buffer, size_t size) {
  if (size < sizeof(raw::FixedLeader))
    throw std::runtime_error(
        "parseFixedLeader: error in stream, buffer size too small");

  raw::FixedLeader const& leader =
      *reinterpret_cast<raw::FixedLeader const*>(buffer);
  ...
  acqConf.blank_after_transmit_distance =
      0.01f * le16toh(leader.blank_after_transmit_distance);
  ...
  acqConf.water_layer_min_ping_threshold =
      1.0f / 255 * leader.water_layer_min_ping_threshold;
  acqConf.water_layer_velocity_threshold =
      0.001 * le16toh(leader.water_layer_velocity_threshold);
  ...
  acqConf.time_between_ping_groups =
      base::Time::fromMicroseconds(milliseconds * 1000);
  ...
  switch (mode & raw::PD0_COORDINATE_SYSTEM_MASK) {
    ...
    default:
      throw std::runtime_error(
          "unexpected value for coordinate_transformation_mode & "
          "raw::PD0_COORDINATE_SYSTEM_MASK");
  }
...
void PD0Parser::parseVariableLeader(uint8_t const* buffer, size_t size) {
  if (size < sizeof(raw::VariableLeader))
    throw std::runtime_error(
        "parseVariableLeader: error in stream, buffer size too small");

  raw::VariableLeader const& msg =
      *reinterpret_cast<raw::VariableLeader const*>(buffer);

  status.seq = static_cast<uint32_t>(msg.seq_low) +
               (static_cast<uint32_t>(msg.seq_high) << 16);
  {
    ...
    status.time = base::Time::fromSeconds(
  ...
  status.pressure_variance =
      100.0f + 10.0f * le32toh(msg.pressure_variance_at_transducer);
  uint64_t milliseconds =
  ...
  for (int i = 0; i < 8; ++i) status.adc_channels[i] = msg.adc_channels[i];
}

void PD0Parser::parseVelocityReadings(uint8_t const* buffer, size_t size) {
  if (size < sizeof(raw::VelocityMessage) +
                 acqConf.cell_count * sizeof(raw::CellVelocity))
    throw std::runtime_error("parseVelocityReadings: buffer size too small");

  raw::VelocityMessage const& msg =
      *reinterpret_cast<raw::VelocityMessage const*>(buffer);
...
void PD0Parser::parseCorrelationReadings(uint8_t const* buffer, size_t size) {
  if (size < sizeof(raw::CorrelationMessage) +
                 acqConf.cell_count * sizeof(raw::CellCorrelation))
    throw std::runtime_error("parseCorrelationReadings: buffer size too small");

  raw::CorrelationMessage const& msg =
      *reinterpret_cast<raw::CorrelationMessage const*>(buffer);
  for (int cell_idx = 0; cell_idx < acqConf.cell_count; ++cell_idx) {
    // This is pre-sized as soon as we know the number of cells in the
    // acquisition process
    CellReading& cell = cellReadings.readings[cell_idx];
...
void PD0Parser::parseIntensityReadings(uint8_t const* buffer, size_t size) {
  if (size < sizeof(raw::IntensityMessage) +
                 acqConf.cell_count * sizeof(raw::CellIntensity))
    throw std::runtime_error("parseIntensityReadings: buffer size too small");

  raw::IntensityMessage const& msg =
      *reinterpret_cast<raw::IntensityMessage const*>(buffer);
  for (int cell_idx = 0; cell_idx < acqConf.cell_count; ++cell_idx) {
    // This is pre-sized as soon as we know the number of cells in the
    // acquisition process
    CellReading& cell = cellReadings.readings[cell_idx];
...
void PD0Parser::parseQualityReadings(uint8_t const* buffer, size_t size) {
  if (size < sizeof(raw::QualityMessage) +
                 acqConf.cell_count * sizeof(raw::CellQuality))
    throw std::runtime_error("parseQualityReadings: buffer size too small");

  raw::QualityMessage const& msg =
      *reinterpret_cast<raw::QualityMessage const*>(buffer);
  for (int cell_idx = 0; cell_idx < acqConf.cell_count; ++cell_idx) {
    // This is pre-sized as soon as we know the number of cells in the
    // acquisition process
    CellReading& cell = cellReadings.readings[cell_idx];
...
void PD0Parser::parseBottomTrackingReadings(uint8_t const* buffer,
                                            size_t size) {
  if (size < sizeof(raw::BottomTrackingMessage))
    throw std::runtime_error(
        "parseBottomTrackingReadings: buffer size too small");

  raw::BottomTrackingMessage const& msg =
      *reinterpret_cast<raw::BottomTrackingMessage const*>(buffer);

  bottomTrackingConf.ping_per_ensemble = le16toh(msg.bottom_ping_per_ensemble);
  bottomTrackingConf.delay_before_reacquiring =
      le16toh(msg.bottom_delay_before_reacquiring);
  bottomTrackingConf.correlation_threshold =
      1.0f / 255 * msg.bottom_correlation_threshold;
  bottomTrackingConf.evaluation_threshold =
      1.0f / 255 * msg.bottom_evaluation_threshold;
  bottomTrackingConf.good_ping_threshold =
      0.01f * msg.bottom_good_ping_threshold;
  bottomTrackingConf.mode = msg.bottom_mode;
  bottomTrackingConf.max_velocity_error =
      1e-3f * le16toh(msg.bottom_max_velocity_error);
  bottomTrackingConf.max_tracking_depth =
      1e-1f * le16toh(msg.max_tracking_depth);

  bottomTracking.time = status.time;
  for (int beam = 0; beam < 4; ++beam) {
    uint32_t value =
        static_cast<uint32_t>(le16toh(msg.bottom_range_low[beam])) +
    ...
    bottomTracking.correlation[beam] =
        1.0f / 255 * msg.bottom_correlation[beam];
    bottomTracking.evaluation[beam] = 1.0f / 255 * msg.bottom_evaluation[beam];
    bottomTracking.good_ping_ratio[beam] =
        1.0f / 255 * msg.bottom_good_ping_ratio[beam];

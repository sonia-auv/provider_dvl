...
Driver::Driver(int max_packet_size, bool extract_last)
    : internal_buffer(new uint8_t[max_packet_size]),
      ...
      m_stream(0),
      m_auto_close(true),
      m_extract_last(extract_last) {
  if (MAX_PACKET_SIZE <= 0)
    std::runtime_error(
        "Driver: max_packet_size cannot be smaller or equal to 0!");
}

Driver::~Driver() {
  delete[] internal_buffer;
  delete m_stream;
  for (set<IOListener*>::iterator it = m_listeners.begin();
...
void Driver::openURI(std::string const& uri) {
  ...
  int mode_idx = -1;
  char const* modes[5] = {"serial://", "tcp://", "udp://", "udpserver://",
                          "file://"};
  for (int i = 0; i < 5; ++i) {
  ...
  if (mode_idx == -1) throw std::runtime_error("unknown URI " + uri);
  ...
  if (mode_idx == 0) {  // serial://DEVICE:baudrate
    if (marker == string::npos)
      throw std::runtime_error(
          "missing baudrate specification in serial:// URI");
...
void Driver::openIPServer(int port, addrinfo const& hints) {
  ...
  if (ret != 0) throw UnixError("cannot resolve server port " + port_as_string);
...
std::pair<uint8_t const*, int> Driver::findPacket(uint8_t const* buffer,
  ...
  if (remaining == 0) return make_pair(buffer + packet_start, packet_size);
...
pair<int, bool> Driver::extractPacketFromInternalBuffer(uint8_t* buffer,
  ...
  while (internal_buffer_size > 0) {
    ...
    // copied in 'buffer'
    if (packet_size) result_size = packet_size;

    if (!packet_size || !m_extract_last) break;
  }
...
pair<int, bool> Driver::readPacketInternal(uint8_t* buffer,
  ...
  if (internal_buffer_size > 0) {
    ...
    // after doPacketExtraction, if a packet is there it has already been
    // copied in 'buffer'
    if (packet_size && !m_extract_last) return make_pair(packet_size, false);
  }
...
int Driver::readPacket(uint8_t* buffer, int buffer_size, int packet_timeout,
  ...
  if (!isValid()) {
    ...
    pair<int, bool> result =
        extractPacketFromInternalBuffer(buffer, buffer_size);
    if (result.first)
      return result.first;
    else
  ...
  if (!m_stream)
    throw std::runtime_error(
        "Driver::writePacket : invalid stream, did you forget to call open ?");
  ...
  while (true) {
    ...
    try {
      // calls select and waits until a new read can be actually performed (in
      // the next
      // while-iteration)
      m_stream->waitRead(
          base::Time::fromMicroseconds(remaining_timeout * 1000));

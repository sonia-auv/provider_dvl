...
struct Time {
 private:
  explicit Time(int64_t microseconds) : microseconds(microseconds) {}
 ...
 public:
  ...
  bool operator!=(Time const& ts) const { return !(*this == ts); }
  bool operator>=(Time const& ts) const { return !(*this < ts); }
  bool operator<=(Time const& ts) const { return !(*this > ts); }
  ...
  Time operator/(int divider) const { return Time(microseconds / divider); }
  Time operator*(double factor) const { return Time(microseconds * factor); }
   ...
   * \param mainFormat Main format to use -- this is passed to strftime and
   *appended by ':' plus the
   *     below seconds resolution if requested by the resolution argument
   **/
  std::string toString(
      base::Time::Resolution resolution = Microseconds,
      const std::string& mainFormat = "%Y%m%d-%H:%M:%S") const {
  ...
  /** Returns this time as an integer number of milliseconds (thus dropping the
   * microseconds) */
  ...
  static Time fromMicroseconds(uint64_t value) { return Time(value); }
  static Time fromMilliseconds(uint64_t value) { return Time(value * 1000); }
  static Time fromSeconds(int64_t value) { return Time(value * UsecPerSec); }
  ...
  /**
    ...
    * Creates a time object from the time values (year, month, day ...) given as
    *integer values. This function can be used
    * when the time values are only available as seperated values in numerical
    *form.
    ...
    *
    */
  static Time fromTimeValues(int year, int month, int day, int hour, int minute,
  ...
  /**
  * Create a time object from an input string, by default all parameters are set
  * to convert the string returned
  ...
  * msec or usec field is present, i.e. the non standard field is separated by
  * ':'
  * \param mainFormat valid format for strptime, e.g."%Y%m%d-%H:%M:%S" which the
  * given time string has
  * \throws std::runtime_error on failure such as a mismatching format
  */
  static Time fromString(const std::string& stringTime,
    ...
    if (resolution > Seconds) {
      ...
      if ((usecsStringLength == 3 || usecsStringLength == 6) &&
          !(usecsStringLength == 3 && resolution > Milliseconds)) {
        // string matches resolutions
      } else {
        throw std::runtime_error(
            "base::Time::fromString failed - resolution does not match "
    ...
    if (NULL == strptime(mainTime.c_str(), mainFormat.c_str(), &tm)) {
      throw std::runtime_error("base::Time::fromString failed- Time-String '" +
                               mainTime + "' did not match the given format '" +
                               mainFormat + "'");
    }
...
inline std::ostream& operator<<(std::ostream& io, base::Time const& time) {
  ...
  io << (microsecs / 1000000) << std::setfill('0') << "." << std::setw(3)
     << (std::llabs(microsecs) / 1000) % 1000 << "." << std::setw(3)
     << (std::llabs(microsecs) % 1000) << std::setfill(' ');

...
class FileGuard {
 ...
 public:
  explicit FileGuard(int fd = -1) : fd(fd) {}
  ~FileGuard() { reset(); };
 ...
 * <ul>
 *   <li> subclass it
 *   <li> give to the Driver constructor the maximum packet size that it can
 *expect
...
class Driver {
 ...
 protected:
   ...
   */
  std::pair<uint8_t const*, int> findPacket(uint8_t const* buffer,
                                            int buffer_size) const;

  /** Internal helper method which reads packets only from the internal buffer
   * (does not access any file descriptor)
   */
  std::pair<int, bool> extractPacketFromInternalBuffer(uint8_t* buffer,
                                                       int out_buffer_size);
  ...
  void openIPClient(std::string const& hostname, int port,
  ...
  */
   ...
   */
  int readPacket(uint8_t* buffer, int bufsize, int packet_timeout,
   ...
   * internal buffer.
   *
   * @throws TimeoutError on timeout or no data, and UnixError on reading
   *problems
   ...
   * @returns always true. The return value is kept for backward compatibility
   *only
   */
  bool writePacket(uint8_t const* buffer, int bufsize,
